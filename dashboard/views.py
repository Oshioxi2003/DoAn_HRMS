...
def employee_dashboard(request):
    ...
    if not request.user.employee:
        messages.info(request, _('Please complete your employee profile'))
...
def manager_dashboard(request):
    ...
    if not request.user.employee or request.user.role != 'Manager':
        messages.error(request, _('You do not have manager access'))
    ...
    if not department:
        messages.warning(request, _('You are not assigned to any department'))
        return redirect('employee_dashboard')
...
def hr_dashboard(request):
    ...
    if request.user.role not in ['HR', 'Admin']:
        messages.error(request, _('You do not have HR access'))
        return redirect('dashboard')
...
def admin_dashboard(request):
    ...
    for i in range(7):
        ...
        day_logs = [log for log in login_logs if log['timestamp__date'] == log_date.date()]
        if day_logs:
...
def attendance_widget(request):
    ...
    if not request.user.employee:
        return JsonResponse({'error': 'No employee profile found'})
