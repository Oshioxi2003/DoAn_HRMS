...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail_subject = 'Activate your HRMS account.'
...
def user_detail(request, user_id):
    ...
    # Get recent activity
    activity_logs = SystemLog.objects.filter(user=user).order_by('-timestamp')[:10]
    
    return render(request, 'accounts/user_detail.html', {
        'user': user,
        'permissions': permissions,
        'activity_logs': activity_logs,
        'can_link_employee': (user.role == 'Employee' and not user.employee)
    })
